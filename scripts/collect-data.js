...
// Configuration
const DOMAIN =
  process.env.VERCEL_URL || process.env.DOMAIN || 'playbacksports.ai';
...
async function collectData() {
  ...
  console.log(
    `🤖 Starting background collection at ${new Date().toISOString()}`
  );
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        `   - ${summary.successfulCollections}/${summary.totalCollections} collections`
      );
      console.log(
        `   - ${summary.totalSlots} slots from ${summary.totalVenues} venues`
      );
      ...
      console.error(
        `❌ Collection failed: ${response.status} ${response.statusText}`
      );
