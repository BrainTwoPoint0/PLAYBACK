...
export default function BookingConfirm({
  ...
  const t = useTranslations('playscanner.booking');
  ...
  // 24h digits, Latin numerals — slot times never localize.
  const timeFormat = {
    hour: '2-digit',
    minute: '2-digit',
    hour12: false,
  ...
  const time = format.dateTime(new Date(slot.startTime), timeFormat);
  ...
  const endTime = format.dateTime(endDt, timeFormat);
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={t('dialogLabel', { venue: slot.venue.name })}
      ...
      >
        ...
        <button
          ...
          onClick={onClose}
          aria-label={t('close')}
          className="absolute end-3 top-3 inline-flex h-11 w-11 items-center justify-center rounded-full text-ink-muted transition-colors hover:text-timberwolf hover:bg-[rgba(214,213,201,0.06)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-timberwolf/60"
        >
        ...
        <div className="mt-4 flex items-center justify-between rounded-lg bg-[rgba(214,213,201,0.04)] p-3">
          <div>
            <div className="text-sm text-ink-muted">{t('time')}</div>
            {/* dir="ltr" pins "18:00 – 19:30" so times don't reorder in RTL */}
            <div dir="ltr" className="text-lg font-semibold text-timberwolf">
              {time} – {endTime}
            </div>
            <div className="text-xs text-ink-muted">
              {t('durationMin', { minutes: slot.duration })}
            </div>
          </div>
          <div className="text-end">
            <div className="text-sm text-ink-muted">
              {isDropIn ? t('perPerson') : t('price')}
            </div>
            {slot.price === 0 ? (
        ...
        <p className="mt-2 text-center text-[11px] text-ink-subtle">
          {t('redirectNotice', { provider: providerConfig.displayName })}
        </p>
      </div>
    </>
  );
