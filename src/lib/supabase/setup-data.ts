...
export async function insertDefaultSports() {
    try {
        ...
        const sportsToInsert = defaultSports.filter(sport =>
            !existingSportNames.includes(sport.name)
        )
...
export async function insertDefaultAchievements() {
    try {
        ...
        const achievementsToInsert = defaultAchievements.filter(achievement =>
            !existingAchievementNames.includes(achievement.name)
        )
        ...
        return {
            ...
            totalAchievements: (existingAchievements?.length || 0) + achievementsToInsert.length
        }
