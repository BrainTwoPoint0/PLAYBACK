...
export class BackgroundCollector {
    ...
    async collectAll(): Promise<CollectionResult> {
        ...
        console.log(`🤖 Starting background collection for ${cities.length} cities, ${daysAhead} days`);
        ...
        for (const city of cities) {
            for (let dayOffset = 0; dayOffset < daysAhead; dayOffset++) {
                ...
                try {
                    ...
                    console.log(`✅ ${city} ${dateString}: ${slots.length} slots from ${this.getUniqueVenues(slots).length} venues`);
                    ...
                    console.error(`❌ ${city} ${dateString}: ${(error as Error).message}`);
                }
            }
        }
        ...
        console.log(`🎯 Collection complete: ${successCount}/${results.length} successful, ${totalSlots} total slots, ${totalTime}ms`);
     ...
     */
    private async collectCityDate(city: string, date: string): Promise<CourtSlot[]> {
        const params = {
    ...
    private getPriceRange(slots: CourtSlot[]): { min: number; max: number } | null {
