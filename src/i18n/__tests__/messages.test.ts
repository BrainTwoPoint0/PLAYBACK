...
describe('en.json', () => {
  it('has content and no empty values', () => {
    // Guards against the catalog being wiped back to the {} placeholder.
    expect(enFlat.size).toBeGreaterThan(0);
...
describe('Arabic digit pinning', () => {
  ...
  it('renders Latin digits in numeric interpolations', () => {
    // Every ar.json plural/number interpolation must use {x, number, latn}.
    // This asserts the convention holds for the catalog's plural keys.
    const arFlat = flatten(ar as Messages);
    const pluralKeys = [...arFlat.entries()].filter(([, v]) =>
      v.includes('plural')
    );
    for (const [key] of pluralKeys.slice(0, 20)) {
      const rendered = t(
        key as Parameters<typeof t>[0],
        { count: 15 } as never
      );
      expect(rendered, key).not.toMatch(/[٠-٩]/);
    }
  });
});
