...
// Static mapping from kebab-cased venue title (after stripping the
// " - Hire A Pitch" suffix) to the real public-site URL slug. Generated by
// crawling hireapitch.com/locations across location filters; refresh with
// `node scripts/discover-hireapitch-slugs.mjs > .../hireapitch-venues.json`
// when the upstream venue list shifts. Hireapitch's marketing slugs (e.g.
// "Marylebone-3G" for "Lisson Green 3G", "Kennington" for "Kennington-Bethwin
// Adventure Playground") aren't derivable from the venue title — without
// this lookup ~37% of click-throughs land on a 404.
const HIREAPITCH_SLUG_MAP = require('./hireapitch-venues.json');
...
class HireAPitchProvider {
  ...
  async checkVenue(id) {
    try {
      ...
      // venue name. Strip it before generating the slug so the displayed
      // venue.name in the UI is clean.
      venueName = venueName.replace(/\s*-?\s*hire\s*a\s*pitch\s*$/i, '').trim();

      // URL slug: prefer the curated public-site slug from
      // HIREAPITCH_SLUG_MAP (covers ~250 UK venues including the 7 broken
      // London cases — Marylebone-3G, Kennington, Canada-Water etc that
      // can't be derived from the title). Fall back to the kebab-cased
      // venue name for anything not yet in the map.
      const kebabSlug =
        venueName
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, '-')
          .replace(/^-|-$/g, '') || String(id);
      const slug = HIREAPITCH_SLUG_MAP[kebabSlug] || kebabSlug;
